...
class SimpleRecycleComponent {

    var driverViewModel: DriverViewModel? = null

    constructor(driverViewModel: DriverViewModel) {
        ...
        if (!view.isCheck.get()!!) {
            ...
            if (index == 0) {
                if (driverViewModel != null) {
                    if (driverViewModel?.mapActivity!!.viewModel?.currentPosition == 0) {

                    } else if (driverViewModel?.mapActivity!!.viewModel?.currentPosition == 3) {

                        driverViewModel?.mapActivity!!.getRoadBookFragment().viewModel?.items!!.clear()
                        driverViewModel?.mapActivity!!.getRoadBookFragment().viewModel?.createMarker(allData!!)
                        driverViewModel?.mapActivity!!.getRoadBookFragment().viewModel?.bottomVisible!!.set(false)
                        driverViewModel?.mapActivity!!.getRoadBookFragment().behavior_by_routes!!.state = BottomSheetBehavior.STATE_COLLAPSED
                    }
                } else {
                ...
                if (driverViewModel != null) {
                    if (driverViewModel?.mapActivity!!.viewModel?.currentPosition == 0) {
                        ...
                        driverViewModel?.mapActivity!!.getRoadBookFragment().viewModel?.createMarker(li)
                        driverViewModel?.mapActivity!!.getRoadBookFragment().viewModel?.title!!.set("DAY$index")
                        driverViewModel?.mapActivity!!.getRoadBookFragment().viewModel?.pointAddress!!.set(li.allRoutepoint)
                        var t = DecimalFormat("0.0").format(li.aboutdis / 1000) + "km"
                        var k = ConvertUtils.duration2Min(li.ridingtime.toInt())
                        driverViewModel?.mapActivity!!.getRoadBookFragment().viewModel!!.distance_andtime.set("距离" + t + "  " + "预计" + k)
                        driverViewModel?.mapActivity!!.getRoadBookFragment().viewModel?.discribe!!.set(li.simpIntroduction)
                        driverViewModel?.mapActivity!!.getRoadBookFragment().viewModel?.id!!.set(li.id.toString())
                        driverViewModel?.mapActivity!!.getRoadBookFragment().viewModel?.bottomVisible!!.set(true)
                        driverViewModel?.mapActivity!!.getRoadBookFragment().behavior_by_routes!!.state = BottomSheetBehavior.STATE_HIDDEN
                    }
                } else {
                    roadBookFirst!!.items!!.clear()
                    var li = this!!.allData!![index - 1]
    ...
    var itemsbinding = OnItemBindClass<Any>()
            .map(RoadBookDistanceEntity::class.java) { itemBinding, position, item ->
                itemBinding.set(BR.road_book_one_entity, R.layout.roadbook_one_layout).bindExtra(BR.simple_model, this@SimpleRecycleComponent)
            }
    ...
    fun initDatas(linear: ArrayList<RoadDetailEntity>, data: HotData?, dex: Int) {
        ...
        CoroutineScope(uiContext).launch {
            ...
            if (roadBookFirst != null) {
                ...
                driverViewModel!!.mapActivity.getRoadBookFragment().viewModel?.dayCount!!.set(data.ridingtime)
            }
...
//        }
        ...
        if (driverViewModel != null) {
            if (driverViewModel?.mapActivity?.viewModel?.currentPosition == 3) {
                if (driverViewModel?.mapActivity!!.getRoadBookFragment().behavior_by_routes!!.state != BottomSheetBehavior.STATE_EXPANDED) {
                    CoroutineScope(uiContext).launch {
                        driverViewModel?.mapActivity!!.getRoadBookFragment().behavior_by_routes!!.state = BottomSheetBehavior.STATE_COLLAPSED
                        delay(500)
