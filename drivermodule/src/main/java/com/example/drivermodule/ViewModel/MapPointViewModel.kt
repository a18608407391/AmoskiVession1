...
//            fr.map_view.invalidate()
    ...
    fun inject(mapPointFragment: MapPointFragment) {
        mapActivity = mapPointFragment.parentFragment as MapFragment
        ...
        when (view.id) {
            ...
            R.id.add_point -> {
                if (mapPointController.pointList.size < 16) {
                    ARouter.getInstance().build(RouterUtils.MapModuleConfig.SEARCH_ACTIVITY).withInt(RouterUtils.MapModuleConfig.SEARCH_MODEL, mapActivity.getDrverFragment().viewModel?.status?.curModel!!).navigation(mapActivity.activity, RESULT_STR)
                } else {
                    Toast.makeText(context, getString(R.string.max_passPoint), Toast.LENGTH_SHORT).show()
                }
            }
...
//                showBottomChangePositionDialog()
            ...
            R.id.edit_finally -> {
                ARouter.getInstance().build(RouterUtils.MapModuleConfig.SEARCH_ACTIVITY).withInt(RouterUtils.MapModuleConfig.SEARCH_MODEL, 3).navigation(mapActivity.activity, EDIT_FINAL_POINT)
            }
        }
    }
    ...
    private fun initView(mapActivity: MapFragment) {
        mapActivity.district_recycleView.layoutManager = LinearLayoutManager(mapActivity.activity)
