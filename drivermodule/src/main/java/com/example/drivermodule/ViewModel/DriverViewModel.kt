...
//            mapActivity.getTeamFragment().viewModel?.addDispose()
            ...
            mapActivity.viewModel?.component!!.isTeam.set(true)
            mapActivity.viewModel?.component!!.Drivering.set(true)
            mapActivity?.viewModel?.changerFragment(1)
            var pos = ServiceEven()
            ...
            if (it.code == 20001) {
                //队伍不存在
                TeamStatus = SoketTeamStatus()
                ARouter.getInstance().build(RouterUtils.TeamModule.TEAM_CREATE).navigation(mapActivity.activity, REQUEST_CREATE_JOIN)
            } else {
                ARouter.getInstance().build(RouterUtils.ActivityPath.LOGIN_CODE).navigation(mapActivity.activity, object : NavCallback() {
    ...
    override fun CheckTeamStatusError(ex: Throwable) {
        mapActivity.dismissProgressDialog()
        mapActivity.viewModel?.selectTab(0)
...
//    var component = DriverComponent()
    ...
    constructor() {
        StringDispose = RxBus.default?.toObservable(String::class.java)?.subscribe {
            if (it == "startNavigation") {
                ...
                mapActivity.viewModel?.selectTab(1)
            }
        }
    ...
    override fun FiveBtnClick(view: View) {
        ...
        when (view.id) {
            ...
            R.id.team_btn -> {
                if (mapActivity.viewModel?.currentPosition == 1) {
                    mapActivity.viewModel?.selectTab(0)
...
//                recycleComponent.initDatas(mapActivity.getRoadBookFragment().road_book_hori_linear)
                if (fr.curPosition != null) {
                    ARouter.getInstance().build(RouterUtils.MapModuleConfig.ROAD_BOOK_ACTIVITY).withSerializable(RouterUtils.MapModuleConfig.ROAD_CURRENT_POINT, fr.curPosition).navigation(mapActivity.activity, REQUEST_LOAD_ROADBOOK)
                }
            }
        }
    }
...
//            ARouter.getInstance().build(RouterUtils.TeamModule.TEAM_CREATE).navigation(mapActivity, REQUEST_CREATE_JOIN)
            ...
            mapActivity.showProgressDialog(getString(R.string.http_loading))
            mapActivity.viewModel?.component!!.Drivering.set(true)
            var team = mapActivity?.viewModel?.mFragments!![1] as TeamFragment
...
//                ARouter.getInstance().build(RouterUtils.MapModuleConfig.NAVIGATION).navigation()
                ...
                mapActivity.viewModel?.changerFragment(2)
...
//                status.curModel = 1
    ...
    fun onComponentClick(view: View) {
        ...
        if (mapActivity.viewModel?.component!!.type.get() == 1) {
...
//                        mapActivity.queryTrack()
    ...
    fun startNavigation() {
        if (mapActivity.viewModel?.currentPosition == 0 || mapActivity.viewModel?.currentPosition == 1) {
            if (status.startDriver.get() == Drivering) {
        ...
        } else if (mapActivity.viewModel?.currentPosition == 2) {
...
//        s.viewModel?.startNavi(NaviLatLng(mapPointController!!.startPoint!!.latitude, mapPointController.startPoint!!.longitude), NaviLatLng(mapPointController.endPoint!!.latitude, mapPointController!!.endPoint!!.longitude), wayPoint)
    ...
    fun startDrive(flag: Boolean) {
        ...
        CoroutineScope(uiContext).launch {
            ...
            if (fr.isAdded && pro != null && pro!!.isShowing) {
                ...
                if (location.isProviderEnabled(LocationManager.GPS_PROVIDER) && location.isProviderEnabled(LocationManager.NETWORK_PROVIDER)) {
                    if (Build.VERSION.SDK_INT >= 23 && mapActivity.activity?.checkSelfPermission(Manifest.permission.ACCESS_FINE_LOCATION) != PackageManager.PERMISSION_GRANTED) {
        ...
        }).observeOn(AndroidSchedulers.mainThread()).subscribe(object : BaseObserver<String>(mapActivity.activity) {
...
//                        s.viewModel?.totalTime?.set(ConvertUtils.formatTimeS(status.second))
    ...
    fun dontHaveOneMetre(cotent: String, leftBtnTv: String, rightBtnTv: String, type: Int) {
        var dia = DialogUtils.createNomalDialog(mapActivity.activity!!, cotent, leftBtnTv, rightBtnTv)
        dia.setOnBtnClickL(OnBtnClickL {
            dia.dismiss()
            if (BaseApplication.MinaConnected!!) {
                var fr = mapActivity?.viewModel?.mFragments!![1] as TeamFragment
    ...
    fun inject(fr: DriverFragment) {
        this.mapActivity = fr.parentFragment as MapFragment
        this.fr = fr
//        roadbookModel = this.mapActivity.getRoadBookFragment().viewModel
        mapActivity.viewModel?.component!!.setHomeStyle()
...
//        fr.basesult.maxOfVisible = n * 6
    ...
    fun startNavi(s: Location, e: Location, wayPoint: ArrayList<LatLng>, flag: Boolean) {
        ...
        if (mapActivity.viewModel?.currentPosition == 2) {
